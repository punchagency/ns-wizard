...
# async def resolve_request(request: ResolveQuery, req: Request):
#     logger.info(f"Received user request: {request.user_request}")
    
#     result = await main(url=request.url, goal=request.user_request, thread_id=request.thread_id)
    
#     base_url = str(req.base_url)
#     image_files = glob.glob("step_*.png")
#     image_files.sort()  
#     cwd =  os.path.join(os.getcwd(), "data")
#     image_urls = [os.path.join(cwd, f) for f in image_files]
...
async def resolve_request(request: ResolveQuery, req: Request):
    ...
    image_urls = []
    if "screenshots" in final_state:
        for local_path in final_state["screenshots"]:
            filename = os.path.basename(local_path)
            image_urls.append(f"{base_url}/data/{filename}")
